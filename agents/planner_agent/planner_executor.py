"""Planner Agent Executor - A2A bridge for planner agent."""
...
class PlannerExecutor(AgentExecutor):
    """An AgentExecutor that runs an ADK-based Agent for planning."""
        ...
        try:
            async for event in self.runner.run_async(
                ...
                if event.is_final_response():
                    ...
                    # Update task in database with execution plan
    ...
    async def execute(
        ...
        logger.debug('[planner_agent] execute called with context: %s', context)
        ...
        logger.debug('[planner_agent] execute exiting')
    ...
    async def cancel(self, context: RequestContext, event_queue: EventQueue):
        ...
        if session_id in self._active_sessions:
            logger.info(
                f'Cancellation requested for active planner session: {session_id}'
            )
            self._active_sessions.discard(session_id)
        else:
            logger.debug(
                f'Cancellation requested for inactive planner session: {session_id}'
            )
