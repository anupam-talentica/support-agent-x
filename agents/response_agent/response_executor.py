"""Response Agent Executor - A2A bridge for response agent."""
...
class ResponseExecutor(AgentExecutor):
    """An AgentExecutor that runs an ADK-based Agent for response synthesis."""
        ...
        try:
            async for event in self.runner.run_async(
                ...
                if event.is_final_response():
                    ...
                    # Update task with output data if task_id is available
                    if hasattr(task_updater, 'task_id') and task_updater.task_id:
                        try:
                            with get_db() as db:
                                ...
                                logger.info(f'Updated task {task_updater.task_id} with response')
    ...
    async def execute(
        ...
        logger.debug('[response] execute exiting')
    ...
    async def cancel(self, context: RequestContext, event_queue: EventQueue):
        ...
        if session_id in self._active_sessions:
            logger.info(
                f'Cancellation requested for active response session: {session_id}'
            )
            self._active_sessions.discard(session_id)
        else:
            logger.debug(
                f'Cancellation requested for inactive response session: {session_id}'
            )
